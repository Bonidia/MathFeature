...
def header(foutput):
	file = open(foutput, 'a')
	file.write("qParameter,Classifier,ACC,std_ACC,MCC,std_MCC,AUC,std_AUC,BACC,std_BACC,kappa,std_kappa,gmean,std_gmean")
	file.write("\n")
	return
...
def evaluate_model_cross(classifier, model, finput):
	...
	scoring = {'ACC': 'accuracy', 'recall': make_scorer(matthews_corrcoef), 'f1': 'roc_auc', 'ACC_B': 'balanced_accuracy', 'kappa': make_scorer(cohen_kappa_score), 'gmean': make_scorer(geometric_mean_score)}
