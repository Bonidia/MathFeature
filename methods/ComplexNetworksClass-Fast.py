...
def complex_network(seq, seqname, foutput, label, ksize, threshold):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', help='Fasta format file, E.g., file.fasta')
    parser.add_argument('-o', '--output', help='CSV format file, E.g., file.csv')
    ...
    parser.add_argument('-n_cpu', '--n_cpu', default=20, help='number of cpus - default = all')
    ...
    records = list(SeqIO.parse(finput, "fasta"))
    pool = multiprocessing.Pool(processes=n_cpu)
    results = pool.starmap(complex_network, [(record.seq, record.name, foutput, label_dataset, ksize, threshold) for record in records])    
    pool.close()
    pool.join()
#############################################################################